...
static void on_cadastrar_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    // Verificar se já existe um disco com o mesmo nome (opcional, mas bom para consistência)
    for (int i = 0; i < numDiscos; i++) {
    ...
    if (numDiscos < MAX_DISCOS) {
        discos[numDiscos] = criarDisco((char*)nome, (char*)autor, preco, estoque);
        if (discos[numDiscos] != NULL) {
            ...
            GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(gtk_widget_get_toplevel(widget)),
                                                     GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
                                                     "Erro ao cadastrar disco (memória)!");
            gtk_dialog_run(GTK_DIALOG(dialog));
            gtk_widget_destroy(dialog);
        }
    } else {
        GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(gtk_widget_get_toplevel(widget)),
                                                 GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK,
                                                 "Limite de discos atingido!");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
    }
    indice_disco_sendo_editado = -1; // Sai do modo de edição, se estiver
...
static void on_carregar_edicao_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
        ...
        gtk_tree_model_get(model, &iter,
                           0, &nome_disco_selecionado,
                           1, &autor_disco_selecionado,
                           2, &preco_disco_selecionado,
                           3, &estoque_disco_selecionado,
                           -1);

        // Encontrar o índice real no array `discos`
        // (Importante se a lista na GUI puder ser reordenada ou filtrada no futuro)
        // Por agora, vamos assumir que a ordem da TreeView corresponde ao array `discos`
        // Uma forma mais robusta seria varrer 'discos' e comparar pelo nome.
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    entry_preco = gtk_entry_new(); // Ideal: GtkSpinButton
    ...
    entry_estoque = gtk_entry_new(); // Ideal: GtkSpinButton
    ...
    // --- Botões de Ação da Lista (Carregar para Edição, Deletar) ---
...
int main(int argc, char **argv) {
    ...
    // discos[numDiscos++] = criarDisco("Kind of Blue", "Miles Davis", 45.0f, 12);
    // discos[numDiscos++] = criarDisco("A Love Supreme", "John Coltrane", 48.0f, 8);
